...
namespace Cyberbear_Events.MachineControl.LightingControl
{
    ...
    public class LightsArdunio
    {
        ...
        public enum Peripheral { Backlight = 6, GrowLight = 1 }; //Which light will be turned on
        ...
        { 
           // PeripheralUpdate.Raise(this, new PeripheralEventArgs(peripheral, value));
            string cmdStr = "";
            if (peripheral == Peripheral.Backlight) //Backlight needs to turn on
            {
                cmdStr = "S2V" + BtoI(value);
            }
            else if (peripheral == Peripheral.GrowLight) //Growlight needs to turn on
            {
